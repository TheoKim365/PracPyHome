...
## <1> 최종 회차 번호를 이용 파일이름 만들기
LAST_NO = 1006    # 최종 회차 
GAMES = 1000000   # 조합하고자 하는 게임 수
LATEST = 10       # 참고하고자 하는 최근 당첨번호 수 
FNAME_HDR = 'Lotto645_won_'   # 파일이름 앞부분 
LOTTO_NUMS_FILE = FNAME_HDR + str(LAST_NO) + '.xlsx'

start_order = LAST_NO - LATEST + 1
...
df2 = df1.loc[start_order : LAST_NO + 1, :]
df2 = df2.astype(int)
...
for numbers, num in games_dict.items():
    if num >= 3:
        # print(numbers, num)
        games_sel_dict[numbers] = num
        
# games_df = pd.DataFrame(games_list)
games_df = pd.DataFrame(games_sel_dict.items())
next_order = LAST_NO + 1
